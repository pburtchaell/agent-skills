...
2. **`box3-voice.yaml`** - Complete voice assistant
   ...
   - Note: wake-word toggle logic targets ESPHome 2024.x; verify against your
     version, as the voice_assistant/micro_wake_word API has since changed.
...
Fonts are **not bundled** — `assets/fonts/README.md` has download instructions
and licensing. Roboto can be auto-downloaded by ESPHome via `gfonts://Roboto`,
but Material Design Icons must be downloaded manually (gfonts cannot fetch MDI).

**Font Configuration** (after downloading MDI per `assets/fonts/README.md`):
```yaml
font:
  - file: "gfonts://Roboto"   # auto-downloaded, no local file needed
    id: roboto_16
    size: 16
...
Icon codepoints reference and download links: `assets/fonts/README.md`
...
Material Design font resources (download instructions in `assets/fonts/README.md`):
- **Roboto** - Material Design typography (auto-download via `gfonts://Roboto`)
- **materialdesignicons-webfont.ttf** - MDI icon font (download manually)
...
1. **Select template** based on project type (base, voice)
